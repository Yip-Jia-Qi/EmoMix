...
#     )
    ...
    # angry_data = sb.dataio.dataset.DynamicItemDataset.from_csv(
    #     csv_path=hparams["angry_data"],
    #     # replacements={"data_root": hparams["data_folder"]},
    # )

    normal_data = []
    if "normal_emotions_list" in hparams:
        ...
        for emotion in normal_emotions_list:
            normal_data.append(
                sb.dataio.dataset.DynamicItemDataset.from_csv(
                    csv_path=f'RAVDESS2Mix_csv/separation/normal/{emotion}.csv'
                    )
                )
    ...
    if "strong_emotions_list" in hparams:
        ...
        for emotion in strong_emotions_list:
            strong_data.append(
                sb.dataio.dataset.DynamicItemDataset.from_csv(
                    csv_path=f'RAVDESS2Mix_csv/separation/strong/{emotion}.csv'
                    )
                )
...
#         )
    ...
    # return train_data, valid_data, test_data, normal_data, strong_data
...
#     else:
    ...
    # Eval
    emotion_list = hparams["normal_emotions_list"]
    for i in range(len(emotion_list)):
            print(emotion_list[i])
            separator.evaluate(normal_data[i], min_key="si-snr")
            separator.save_results(normal_data[i], f'n_{emotion_list[i]}')
    
    emotion_list = hparams["strong_emotions_list"]
    for i in range(len(emotion_list)):
            print(emotion_list[i])
            separator.evaluate(strong_data[i], min_key="si-snr")
            separator.save_results(strong_data[i], f's_{emotion_list[i]}')
    print("done")
